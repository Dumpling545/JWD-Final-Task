...
public class GoToRoomPage implements Command {
	...
	private static final String RATING_INFO_ATTRIBUTE_KEY = "ratingInfo";
			...
			throws ServletException, IOException
	{
		...
		try {
			...
			Optional<RoomFeature> feature =
					roomService.getRoomFeatureById(roomId);
			if (feature.isPresent()) {
				request.setAttribute(FEATURE_ATTRIBUTE_KEY, feature.get());
			}
			RoomRatingInformation roomRatingInformation =
					reviewService.getRoomRatingInformation(roomId);
			request.setAttribute(RATING_INFO_ATTRIBUTE_KEY,
					roomRatingInformation);
